...
function fetchAndDisplayTeamInfo() {
  var urlParams, leagueId, _ref, teams, error, tableContainer, table;
  ...
  return regeneratorRuntime.async(function fetchAndDisplayTeamInfo$(_context) {
    while (1) {
      switch (_context.prev = _context.next) {
        case 0:
          _context.prev = 0;
          // Récupérer l'identifiant de la ligue à partir de l'URL
          urlParams = new URLSearchParams(window.location.search);
          leagueId = urlParams.get('id_ligue'); // Exécuter la requête SQL pour récupérer les informations des équipes de la ligue spécifiée dans l'URL

          _context.next = 5;
          return regeneratorRuntime.awrap(supabase.from('equipe').select('*').eq('id_ligue', leagueId));
